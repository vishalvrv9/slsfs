...
public:
    ...
    auto perform(slsfs::jsre::request_parser<slsfs::base::byte> const& input) -> slsfs::base::buf override
    {
        ...
        {
            slsfs::log::log("_data_ cass perform_single_request get data");
        ...
        case slsfs::jsre::operation_t::read:
        {
            ...
            slsfs::log::log("_data_ perform_single_request sending: {}, {}, {}, {}", blockid, offset, size, slsfs::pack::ntoh(size));
            for (std::shared_ptr<slsfs::storage::interface> host : hostlist_)
                response = host->read_key(input.uuid(), blockid, offset, size);

            slsfs::log::log("_data_ perform_single_request read from ssbd");
            break;
        }
        }
        return response;
