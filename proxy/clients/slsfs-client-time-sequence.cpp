    ...
    -> std::pair<std::list<double>, std::chrono::nanoseconds>
{
    ...
    timer->async_wait(
        [timer, &io] (boost::system::error_code error) {
            switch (error.value())
            {
    ...
    {
        proxy_sockets.emplace_back(io);

        boost::system::error_code ec;
        boost::asio::connect(proxy_sockets.back(), proxy_endpoint.at(i), ec);
        BOOST_LOG_TRIVIAL(debug) << "connecting " << proxy_sockets.back().remote_endpoint() << " " << ec.message() ;
    }

    static std::mt19937 engine(std::random_device{}());
    std::chrono::time_point const end_time = std::chrono::system_clock::now() + (total_duration * 1s);
    std::chrono::time_point const start_time = std::chrono::system_clock::now();

    for (int i = 0; i < times; i++)
    {
        boost::asio::post(
            io,
            [&io, &rwdist, &genpos, &buf, &proxy_uuid, &proxy_sockets, start_time, genname, timer, i, times, end_time] {
                ...
                long int r = record(
                    [&rwdist, &genpos, &buf, &proxy_uuid, &proxy_sockets, start_time, genname, timer, i, times, end_time] {
                        BOOST_LOG_TRIVIAL(debug) << "write index " << i;
                        ...
                        int limit = proxy_uuid.size();

                        if (std::chrono::system_clock::now() - start_time >=  0s)
                            limit = 1;
                        if (std::chrono::system_clock::now() - start_time >= 10s)
                            limit = 2;
                        if (std::chrono::system_clock::now() - start_time >= 20s)
                            limit = 3;
                        if (std::chrono::system_clock::now() - start_time >= 30s)
                            limit = 4;
                        if (std::chrono::system_clock::now() - start_time >= 40s)
                            limit = 5;
                        if (std::chrono::system_clock::now() - start_time >= 50s)
                            limit = 6;
                        if (std::chrono::system_clock::now() - start_time >= 60s)
                            limit = 5;
                        if (std::chrono::system_clock::now() - start_time >= 70s)
                            limit = 4;
                        if (std::chrono::system_clock::now() - start_time >= 80s)
                            limit = 3;
                        if (std::chrono::system_clock::now() - start_time >= 90s)
                            limit = 2;
                        if (std::chrono::system_clock::now() - start_time >= 100s)
                            limit = 1;
                        ...
                        BOOST_LOG_TRIVIAL(debug) << "sending " << i << " and picked=" << index;
