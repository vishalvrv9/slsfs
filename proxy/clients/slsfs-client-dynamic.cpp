...
auto get_uuid (boost::asio::io_context &io_context, std::string const& child) ->
    boost::asio::ip::tcp::resolver::results_type
{
    static oneapi::tbb::concurrent_hash_map<std::string, boost::asio::ip::tcp::resolver::results_type> cache;

    decltype(cache)::accessor acc;

    if (cache.find(acc, child))
        return acc->second;
    ...
    boost::asio::ip::tcp::resolver::results_type result = resolver.resolve(host, port);
    cache.emplace(child, result);

    return result;
}


auto setup_slsfs(boost::asio::io_context &io_context)
    -> std::pair<std::vector<slsfs::uuid::uuid>, std::vector<boost::asio::ip::tcp::resolver::results_type>>
{
    zk::client client = zk::client::connect("zk://zookeeper-1:2181").get();
...
{
    oneapi::tbb::concurrent_vector<long int> records;
    ...
    timer->async_wait(
        [&io_context, timer] (boost::system::error_code error) {
            switch (error.value())
            {
    ...
    for (int i = 0; i < times; i++)
    {
        boost::asio::post(
            io_context,
            [&records, &rwdist, &genpos, &buf, &io_context, &genname, timer, i, times, end_time] {
                long int r = record(
                    [&rwdist, &genpos, &buf, &io_context, &genname, timer, i, times, end_time] {
                        if (i + 1 >= times)
                            timer->cancel();
                        ...
                        for (;;)
                        {
                            ...
                            thread_local static
                                std::pair<std::vector<slsfs::uuid::uuid>, std::vector<boost::asio::ip::tcp::resolver::results_type>>
                                proxy_info = setup_slsfs(io_context);

                            if (std::chrono::system_clock::now() - last_update > 1s)
                            {
                                proxy_info = setup_slsfs(io_context);
                                last_update = std::chrono::system_clock::now();
                            }

                            auto && [proxy_uuid, proxy_endpoint] = proxy_info;

                            slsfs::pack::key_t key = genname();
                            int index = pick_proxy(proxy_uuid, key);

                            tcp::socket proxy_socket{io_context};
                            boost::asio::connect(proxy_socket, proxy_endpoint.at(index));
                            ...
                            if constexpr (false /* no create file */)
                            {
                                ...
                                boost::asio::write(proxy_socket, boost::asio::buffer(mptr->data(), mptr->size()));
                                ...
                                boost::asio::read(proxy_socket, boost::asio::buffer(headerbuf.data(), headerbuf.size()));

                                resp->header.parse(headerbuf.data());
                                BOOST_LOG_TRIVIAL(debug) << "meta resp:" << resp->header;

                                std::string data(resp->header.datasize, '\0');
                                boost::asio::read(proxy_socket, boost::asio::buffer(data.data(), data.size()));
                                BOOST_LOG_TRIVIAL(debug) << data ;
                            }

                            BOOST_LOG_TRIVIAL(trace) << "write to proxy: " << proxy_socket.remote_endpoint() << " with header=" << ptr->header;
                            boost::system::error_code ec;
                            boost::asio::write(proxy_socket, boost::asio::buffer(pbuf->data(), pbuf->size()), ec);
                            if (ec)
                            {
                                BOOST_LOG_TRIVIAL(error) << "got error on write. retry";
                                continue;
                            }


                            BOOST_LOG_TRIVIAL(trace) << "read header from " << proxy_socket.remote_endpoint();
                            slsfs::pack::packet_pointer resp = std::make_shared<slsfs::pack::packet>();
                            std::vector<slsfs::pack::unit_t> headerbuf(slsfs::pack::packet_header::bytesize);
                            boost::asio::read(proxy_socket, boost::asio::buffer(headerbuf.data(), headerbuf.size()), ec);
                            if (ec)
                            {
                                BOOST_LOG_TRIVIAL(error) << "got error on read header. retry";
                                continue;
                            }

                            resp->header.parse(headerbuf.data());

                            BOOST_LOG_TRIVIAL(trace) << "read body: " << proxy_socket.remote_endpoint() << " header=" << resp->header;

                            std::string data(resp->header.datasize, '\0');
                            boost::asio::read(proxy_socket, boost::asio::buffer(data.data(), data.size()), ec);
                            if (ec)
                            {
                                BOOST_LOG_TRIVIAL(error) << "got error on read body. retry";
                                continue;
                            }
    ...
    for (unsigned int i = 0; i < 1; i++) // max 6 proxys
        ths.emplace_back(
            [&io_context] () {
                io_context.run();
            });
